...
(define interleave3
  (lambda (lz1 lz2 lz3)
    (cond ((empty-lzl? lz1) (interleave lz2 lz3))
          ((empty-lzl? lz2) (interleave lz1 lz3))
          ((empty-lzl? lz3) (interleave lz1 lz2))
          ((and(empty-lzl? lz1) (empty-lzl? lz2)) lz3)
          ((and(empty-lzl? lz2) (empty-lzl? lz3)) lz1)
          ((and(empty-lzl? lz1) (empty-lzl? lz3)) lz2)
          (else (cons-lzl (head lz1)
                          (lambda () (interleave3 lz2 lz3 (tail lz1))))))))
...
(define biasedInterleave
  (lambda (lz1 lz2)
    (if (empty-lzl? lz1)
        lz2
        (cons-lzl (head lz1)
                  (lambda () (addMore1 (tail lz1) lz2))))))


;; Signature: addMore1(lz1, lz2)
;; Type: Lzl(T)*Lzl(T)->Lzl(T)
...
(define addMore1
  (lambda (lz1 lz2)
    (if (empty-lzl? lz1)
        lz2
        (cons-lzl (head lz1)
                  (lambda () (addMore2 lz2 (tail lz1)))))))



;; Signature: addMore2(lz1, lz2)
;; Type: Lzl(T)*Lzl(T)->Lzl(T)
...
(define addMore2
  (lambda (lz1 lz2)
    (if (empty-lzl? lz1)
        lz2
        (cons-lzl (head lz1)
                  (lambda () (biasedInterleave lz2 (tail lz1)))))))
