...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Some lazy lists
; A call (testsmall <number>) or (testbig <number>) will create a lazy tree.
...
(define take-lzt
  (lambda (lzt n)
    (take-bfs-lzt lzt n 0)
  )
)

(define take-bfs-lzt
  (lambda (lzt n depth)
    (if (or (= n 0) (empty-lzt? lzt)) '()
        (let ((nextDepthNodes (lzt->nth-level lzt depth)))
          (let ((nodesAmount (length nextDepthNodes)))
            (cond
              ((< nodesAmount n) (append nextDepthNodes (take-bfs-lzt lzt (- n nodesAmount) (+ depth 1))))
              ((= nodesAmount 0) '())
              (else (take nextDepthNodes n)))))))) 