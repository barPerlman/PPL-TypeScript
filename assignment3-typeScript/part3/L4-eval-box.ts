...
import { applyEnv, applyEnvBdg, globalEnvAddBinding, makeExtEnv, setFBinding,
            theGlobalEnv,GlobalEnv, Env,persistentEnv,isGlobalEnv,isExtEnv,ExtEnv,Frame,unbox,FBinding,generateBodyId,getEnvId } from "./L4-env-box";
import { isEmptySExp, isSymbolSExp, isClosure, isCompoundSExp, makeClosure, makeCompoundSExp, Closure, 
         CompoundSExp, EmptySExp, makeEmptySExp, Value, SExp, valueToString } from "./L4-value-box";
...
export const drawEnvDiagram = (pEnv: {}): Tree | Error => {
    let graph = new Graph();
    Object.keys(pEnv).map((i) =>{ graph=drawNodes(pEnv[i],graph)});
    Object.keys(pEnv).map((i) =>{ graph=drawEdges(pEnv[i],graph)});
    let randomId = '_' + Math.random().toString(36).substr(2, 9);
    return isError(graph) ? graph : {tag: "Tree", rootId: randomId, graph: graph}
}
 
 export const evalParseDraw = (s: string): string | Error => {
    let parsedEval = evalParse(s);
    if(isError(parsedEval))
        return parsedEval;
    else
    {
        let persistentEnvDiagram = drawEnvDiagram(persistentEnv);
        if(isError(persistentEnvDiagram)) 
            return Error("Error happend while drawing the environment Diagrams."); 
        else 
            return dot.write(persistentEnvDiagram.graph);
    }
 }

const drawNodes = (env:Env,graph:Graph):Graph =>     
    isExtEnv(env) ?  graph.setNode(getEnvId(env),{ label: "{"+getEnvId(env)+"|"+env.frame.fbindings.map(drawingFBindingLabel).join("|")+"}",
                    shape:"Mrecord"}) :
    isGlobalEnv(env) ? graph.setNode(env.envId, { label: "{"+env.envId+"|"+unbox(env.frame).fbindings.map(drawingFBindingLabel).join("|")+"}", 
                shape:"Mrecord"}) : graph;                                   

const drawingFBindingLabel = (binding:FBinding):string =>
    drawingLeftLabel(binding) + ":" + drawingRightLabel(binding);
 
const drawingRightLabel = (fbinding:FBinding):string => // "\\l" means new line
    isClosure(unbox(fbinding.val)) ? "\\l" : valueToString(unbox(fbinding.val))+"\\l"; 

 const drawingLeftLabel = (binding:FBinding):string =>
     isClosure(unbox(binding.val)) ? "<" + binding.var + ">" + binding.var : binding.var; 
 
 const drawEdges = (env:Env,graph:Graph):Graph =>
     isExtEnv(env) ? drawEdgesExtEnv(env,graph) :
     isGlobalEnv(env) ? drawEdgesGlobalEnv(env,graph) : graph;
 
 const drawEdgesGlobalEnv = (env:GlobalEnv, graph:Graph):Graph =>
     unbox(env.frame).fbindings.reduce( (acc,curr) => acc = drawEdgesFBinding(env,curr,acc), graph);
 
const drawEdgesExtEnv = (env:ExtEnv, graph:Graph):Graph =>
    env.frame.fbindings.reduce( (acc, curr) => acc = drawEdgesFBinding(env, curr, acc), graph).setEdge(env.envId,env.env.envId).setNode(env.env.envId+"_link",
    {label:env.env.envId,shape:"plaintext"}).setEdge(env.envId,env.env.envId+"_link",{style:"dashed"});

const drawEdgesFBinding = (env:Env,binding:FBinding,graph:Graph):Graph => {
    let unBox = unbox(binding.val);
    return isClosure(unBox) ? drawClosure(env,unBox,graph,binding.var) : graph
}

const drawClosure = (from:Env, val:Closure, graph:Graph,name:string):Graph => {
    let bodyId = generateBodyId();
    return graph.setNode(bodyId, {label: "{<0>\u25EF\u25EF\\l|p:"+map((currVar)=>currVar.var,val.params).join(",")+"\\l| b:"+map((currVar)=>unparse(currVar),val.body).join("\n")+"\\l}", shape: "record"})
         .setEdge(bodyId, from.envId,{tailport:bodyId,headerport:from.envId})
         .setEdge(from.envId,bodyId,{tailport:name,headerport:0})
}
