...
;; [Empty -> void]
*/
import {sort, chain, concat, filter, map, uniq, reduce } from "ramda";
...
export const makeUnionTExp = (exps: TExp[]): UnionTExp => ({tag: "UnionTExp", params: exps});
...
*/
const parseCompoundTExp = (texps: any[]): UnionTExp | ProcTExp | Error => {
    const unionTExpPos = texps.indexOf('|');	
    if(unionTExpPos>0)	
...
const parseUnionTExp = (texps : any[]) : UnionTExp | Error => {
    let tempCheck = checkIfUnionType(texps);
    if(isError(tempCheck))
        return tempCheck;
    else{
        let typesArray = reduce((acc, curr) => {
            if(curr === '|')
                return acc;
            else 
        ...
        let typesArrayAfterParsed = reduce((acc, curr) => {
            if(curr === '|')
                return acc;
            else if (isError(acc))
                return acc;
            let parsedTExp = parseTExp(curr);
            if(isError(parsedTExp))
                return parsedTExp;
            else 
                return acc.concat(parsedTExp);
        }, [], typesArray);
        
        let typesArrayAfterParsedAndSort = sort((exp1: TExp,exp2: TExp): number => exp1.tag < exp2.tag ? -1 : exp1.tag === exp2.tag ? 0 : +1, typesArrayAfterParsed);

        let typesWithoutRiturns = reduce((acc, curr) => {
            if(checkDups(curr,acc))
                return acc;
            else 
                return acc.concat(curr);
        }, [], typesArrayAfterParsedAndSort);
        
        if(isError(typesWithoutRiturns))
            return typesWithoutRiturns;
        else
            return makeUnionTExp(typesWithoutRiturns);
    }
}
...
const checkIfUnionType = (texps : any[]) : number | Error => {
    var checkUnionType = reduce((acc:number | Error, curr) => {
        if(isError(acc))
            return acc;
        else if(acc === 0){
                if(curr === '|')
                    return Error('Wrong union type expression!`')
                else 
                    return 1; // indicates weither the index is odd or even
        }
        else if(curr === '|'){
                return 0; // indicates weither the index is odd or even
        }
        else 
            return Error('Wrong union type expression!`')   
    },0, texps);
    return checkUnionType;
}

const safeMakeProcTExp = (args: Array<TExp | Error>, returnTE: Error | TExp): Error | ProcTExp | UnionTExp =>
    isError(returnTE) ? returnTE :
    hasNoError(args) ? makeProcTExp(args, returnTE) :
    hasNoError(args)? makeUnionTExp(args) :
...
const unParseUnionTExp = (texp: UnionTExp):string | Error =>	
    "("+texp.params.map((tExp: TExp) =>unparseTExp(tExp)).join(" | ")+")";
